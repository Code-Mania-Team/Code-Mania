...
const CodingExamPage = () => {
  ...
  const runCode = async () => {
    ...
    try {
      ...
      const results = {
        passed: isCorrect,
        output: isCorrect ? challenge.testCases[0].expectedOutput : "Code doesn't match expected output",
        testCases: challenge.testCases.map(testCase => ({
          ...testCase,
          passed: isCorrect,
          actualOutput: isCorrect ? testCase.expectedOutput : "Code doesn't match expected output"
        }))
      };
  ...
  const calculateScore = () => {
    ...
    const maxPoints = examData.challenges.reduce((sum, challenge) => sum + challenge.points, 0);
          ...
          <div className={styles.heroContent}>
            ...
            <div className={styles.resultsContainer}>
              ...
              <div className={styles.resultsDetails}>
                <p>
                  Challenges Completed: {score.completed}/{examData.challenges.length}
                </p>
                <p>
        ...
        <div className={styles.questionContainer}>
          ...
          <div style={{ display: 'flex', gap: '2rem', marginBottom: '1rem', width: '100%' }}>
            ...
            <div style={{ flex: '1', minWidth: '450px', maxWidth: '600px' }}>
              ...
              <div style={{
                ...
                <h4 style={{ color: '#3b82f6', marginBottom: '0.5rem' }}>Test Cases:</h4>
                {challenge.testCases.map((testCase, index) => (
                  <div key={index} style={{ marginBottom: '0.5rem' }}>
                    <p style={{ color: '#e2e8f0', margin: '0', fontSize: '0.9rem' }}>
                      <strong>Test {index + 1}:</strong> {testCase.description}
                    </p>
                    <p style={{ color: '#94a3b8', margin: '0.25rem 0 0 1rem', fontSize: '0.85rem' }}>
              ...
              {showHints && (
                <div style={{
                  ...
                  <h4 style={{ color: '#f59e0b', marginBottom: '0.5rem' }}>Hints:</h4>
                  <ul style={{ margin: 0, paddingLeft: '1.5rem' }}>
                    {challenge.hints.map((hint, index) => (
                      <li key={index} style={{ color: '#e2e8f0', marginBottom: '0.25rem' }}>
                        {hint}
                      </li>
                    ))}
                  </ul>
          ...
          {testResults && (
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', marginBottom: '1rem' }}>
                {testResults.passed ? (
                  <CheckCircle style={{ color: '#10b981', marginRight: '0.5rem' }} />
                ) : (
                  <XCircle style={{ color: '#ef4444', marginRight: '0.5rem' }} />
                )}
                <span style={{ 
              ...
              <div style={{ marginBottom: '1rem' }}>
                <h5 style={{ color: '#e2e8f0', marginBottom: '0.5rem' }}>Test Results:</h5>
                  ...
                  <div key={index} style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', marginBottom: '0.25rem' }}>
                      {testCase.passed ? (
                        ...
                        <XCircle size={16} style={{ color: '#ef4444', marginRight: '0.5rem' }} />
                      )}
                      <span style={{ color: '#e2e8f0', fontSize: '0.9rem' }}>
                        Test {index + 1}: {testCase.passed ? 'Passed' : 'Failed'}
                      </span>
                    </div>
                    <p style={{ color: '#94a3b8', margin: '0.25rem 0 0 1.5rem', fontSize: '0.85rem' }}>
                      Expected: "{testCase.expectedOutput}"
                    </p>
                    {!testCase.passed && (
                      <p style={{ color: '#ef4444', margin: '0.25rem 0 0 1.5rem', fontSize: '0.85rem' }}>
          ...
          <div className={styles.examNavigation}>
            {testResults?.passed && (
              ...
              >
                {currentChallenge < examData.challenges.length - 1 ? 'Next Challenge' : 'Finish Exam'}
              </button>
            )}
          </div>
        </div>
