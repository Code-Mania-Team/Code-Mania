...
class AccountController {
  ...
  async requestOtp(req, res) {
    try {
      ...
      const response = await this.accountService.requestSignupOtp(
        email,
        password,
      );
  ...
  async setUsernameAndCharacter(req, res) {
    ...
    try {
      const updated = await this.user.setUsernameandCharacter(
        user_id,
        username,
        character_id,
        full_name,
  ...
  async login(req, res) {
    try {
      ...
      const authUser = await this.accountService.loginWithPassword(
        email,
        password,
      );
      if (!authUser) {
      ...
      if (err?.message === "Email not registered yet") {
        return res
          .status(404)
          .json({ success: false, message: "Email not registered yet" });
      }
      return res.status(500).json({ success: false, message: err.message });
    }
  }
  ...
  async googleLogin(req, res) {
    ...
    const data = await this.accountService.googleLogin(
      id,
      emails[0].value,
      provider,
    );
  ...
  async refresh(req, res) {
    try {
      ...
      if (
        ...
        rawRefreshToken.trim().startsWith("{")
      ) {
        try {
  ...
  async updateProfile(req, res) {
    ...
    try {
      const updated = await this.user.updateProfile(userId, {
        username,
        full_name,
      });
      if (!updated) {
