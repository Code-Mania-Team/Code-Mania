...
class UserGameDataController {
  ...
  async learningData(req, res) {
    try {
      ...
      const programming_language = req.query.programming_language;
      ...
      return res.status(200).json({
        ...
        xpEarned: rows.reduce((sum, r) => sum + (r.xp_earned || 0), 0),
      });
    } catch (err) {
  ...
  async gameData(req, res) {
    try {
      ...
      if (err.message === "xp_earned must be a number") {
        return res
          .status(400)
          .json({ success: false, message: "xp_earned must be a number" });
      }
