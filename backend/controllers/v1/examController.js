...
class ExamController {
  ...
  async listProblems(req, res) {
    try {
      ...
      return res
        .status(500)
        .json({ success: false, message: "Failed to list exam problems" });
    }
  }
  ...
  async getProblem(req, res) {
    try {
      ...
      if (!Number.isFinite(problemId)) {
        return res
          .status(400)
          .json({ success: false, message: "Invalid problemId" });
      }
      ...
      return res
        .status(500)
        .json({ success: false, message: "Failed to fetch exam problem" });
    }
  }
  ...
  async startAttempt(req, res) {
    try {
      ...
      return res
        .status(500)
        .json({ success: false, message: "Failed to start attempt" });
    }
  }
  async submitAttempt(req, res) {
    try {
      ...
      if (!Number.isFinite(attemptId)) {
        return res
          .status(400)
          .json({ success: false, message: "Invalid attemptId" });
      }
      ...
      return res
        .status(500)
        .json({ success: false, message: "Failed to submit exam attempt" });
    }
  }
  ...
  async status(req, res) {
    try {
      ...
      return res
        .status(500)
        .json({ success: false, message: "Failed to fetch exam status" });
    }
  }
}
