    ...
    try {
        ...
        if (course) {
            ...
            for (const courseName of ['python', 'javascript', 'cpp']) {
                const courseExercises = await ExerciseModel.getExercises(courseName, true);
...
router.post('/admin/exercises', requireAdmin, async (req, res) => {
    try {
        ...
        const requiredFields = ['course', 'exercise_id', 'title', 'validation_mode', 'dialogue'];
...
router.post('/admin/exercises/import/:course', requireAdmin, async (req, res) => {
    try {
        const { course } = req.params;
        ...
        const importedExercises = await ExerciseModel.bulkImportExercises(course, exercises);
        res.status(201).json({ 
            success: true, 
